...
# Update: Instead of adding a publisher for position, changed the QuadrotorController class so that the roll rate(bodyrate.x) itself gets published in the main loop
import rospy
...
# function for publishing the commanded roll rate
def publish_roll_command(self, roll_command,attitude_command_pub): 
...
# Funtion for publishing the target position
def publish_position_command(self, position_command,position_command_pub): # currently unused 

    position_target = PositionTarget()
    position_target.header.stamp = rospy.Time.now()
    position_target.coordinate_frame = PositionTarget.FRAME_BODY_NED
    # Mask to ignore the velocities, accelerations, yaw and yaw_rate
    ...
    position_command_pub.publish(position_target)
...
if __name__ == "__main__":
    ...
    position_command_pub = rospy.Publisher('/mavros/setpoint_raw/local', PositionTarget, queue_size=10) # currently unused
